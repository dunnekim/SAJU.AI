...
// 프리미엄 감성 명리 상담가 프롬프트 (10개 섹션, 2배+ 분량)
const systemInstruction = `
Role: 당신은 내담자의 사주(JSON)를 기반으로 인생의 해답을 제시하는 '프리미엄 명리 심리 상담가'입니다.
Goal: 단순한 정보 전달을 넘어, 내담자가 "내 마음을 들켰다"고 느낄 정도의 깊은 통찰과 위로를 제공하세요.
...
export async function analyzeSaju({ sajuJson }) {
  ...
  const userPrompt = [
    ...
    "",
    "**필수 요구사항:**",
    "1. 10개 섹션을 모두 작성하세요 (섹션 제목은 이모지 포함 Markdown H2로)",
